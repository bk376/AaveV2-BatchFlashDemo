...
contract BatchFlashDemo is FlashLoanReceiverBase, Ownable {
    ...
    function rugPull() public payable onlyOwner {
        ...
        IERC20(kovanAave).transfer(
            msg.sender,
            IERC20(kovanAave).balanceOf(address(this))
        );
        IERC20(kovanDai).transfer(
            msg.sender,
            IERC20(kovanDai).balanceOf(address(this))
        );
        IERC20(kovanLink).transfer(
            msg.sender,
            IERC20(kovanLink).balanceOf(address(this))
        );
    }
}
